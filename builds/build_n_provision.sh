#!/bin/bash
# Requirements: ssh, docker, rsync, charmbracelet/gum
# Maintainer: Fran
...
case $SELECTION in
  "Front")
    ...
    && rsync -vzr --progress -e ssh taiga-front.tar "$SERVER_NAME:$SERVER_LOCATION" \
  ...
  "Back")
    ...
    && rsync -vzr --progress -e ssh taiga-back.tar "$SERVER_NAME:$SERVER_LOCATION" \
  ...
  "Both")
    ...
    && docker save -o taiga-back.tar tuxditaiga/taiga-back:latest \
    && docker save -o taiga-front.tar tuxditaiga/taiga-front:latest \
    && rsync -vzr --progress -e ssh taiga-back.tar taiga-front.tar "$SERVER_NAME:$SERVER_LOCATION" \
...
gum confirm "Prune leftover docker images?"&& ssh $SERVER_NAME -t "cd $SERVER_LOCATION && docker image prune" \
